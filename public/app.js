...
async function api(url, method = 'GET', body = null) {
  return demoApi(url, method, body);
}
...
function renderSpectrumCharts() {
  ...
  nodeDataCharts.spectrum = new Chart(specEl, {
    type:'bar',
    data:{labels:fl, datasets:[{label:'Vibration Spectrum (mm/s)', data:Array.from({length:50},()=>+(Math.random()*10).toFixed(2)), backgroundColor:'#6366f1aa', borderColor:'#6366f1', borderWidth:1}]},
  ...
  nodeDataCharts.ultra = new Chart(ultraEl, {
    type:'bar',
    data:{labels:fl, datasets:[{label:'Ultrasound Spectrum (dB)', data:Array.from({length:50},()=>+(Math.random()*8).toFixed(2)), backgroundColor:'#06b6d4aa', borderColor:'#06b6d4', borderWidth:1}]},
    options:{responsive:true, maintainAspectRatio:false, animation:false, plugins:{legend:{display:false}}, scales:{y:{beginAtZero:true,grid:{color:'#f1f5f9'}},x:{ticks:{maxTicksLimit:10},grid:{display:false}}}}
  });
}
...
async function openCongregation(nodeId) {
  ...
  document.getElementById('congNodeInfo').innerHTML = `
    <div style="display:flex;align-items:center;justify-content:center;width:56px;height:56px;background:linear-gradient(135deg,#6366f1,#8b5cf6);border-radius:14px;flex-shrink:0;">${NODE_ICON_SVG}</div>
    <div>
  ...
  grid.innerHTML = congMotors.map(m => {
    ...
    <div class="motor-card ${m.motor_name?'saved':''}">
      <div class="motor-health-bar">
        ...
        <div class="health-track"><div class="health-fill" style="width:${health}%;background:${healthColor};"></div></div>
      </div>
      ...
      <div class="matrix-row"><label>Health Score</label>
        <div style="display:flex;align-items:center;gap:8px;flex:1;">
          <input type="range" id="mhealth_${m.id}" min="0" max="100" value="${health}" oninput="document.getElementById('mhealthval_${m.id}').textContent=this.value+'%'" style="flex:1;" />
      ...
      <div class="matrix-row"><label>Machine Type</label>
        <select id="mtype_${m.id}">${['Pump','Motor','Compressor','Fan/Blower','Gearbox','Conveyor','Crusher','Turbine','Generator','Agitator','Centrifuge','Other'].map(t=>`<option ${(m.machine_type||'')=== t?'selected':''}>${t}</option>`).join('')}</select>
      </div>
      ...
      <div class="matrix-row"><label>RPM Type</label>
        <select id="mrpmtype_${m.id}">${['Constant','Variable'].map(t=>`<option ${(m.rpm_type||'Constant')===t?'selected':''}>${t}</option>`).join('')}</select>
      </div>
      <div class="matrix-row"><label>Rotor Type</label>
        <select id="mrotor_${m.id}">${['Squirrel Cage','Wound Rotor','Permanent Magnet','Other'].map(t=>`<option ${(m.rotor_type||'')=== t?'selected':''}>${t}</option>`).join('')}</select>
      </div>
      ...
      <div class="matrix-row"><label>Coupling Type</label>
        <select id="mcoupling_${m.id}">${['Direct','Belt','Gear','Flexible','Fluid','Chain','Other'].map(t=>`<option ${(m.coupling_type||'')=== t?'selected':''}>${t}</option>`).join('')}</select>
      </div>
      ...
      <div class="matrix-row"><label>DE Manufacturer</label>
        <select id="mbdemake_${m.id}">${['SKF','FAG','NSK','NTN','Timken','INA','Other'].map(t=>`<option ${(m.bearing_de_make||'')=== t?'selected':''}>${t}</option>`).join('')}</select>
      </div>
      <div class="matrix-row"><label>DE Type</label>
        <select id="mbdetype_${m.id}">${['Ball','Roller','Spherical','Tapered','Needle','Other'].map(t=>`<option ${(m.bearing_de_type||'')=== t?'selected':''}>${t}</option>`).join('')}</select>
      </div>
      ...
      <div class="matrix-row"><label>NDE Manufacturer</label>
        <select id="mbndemake_${m.id}">${['SKF','FAG','NSK','NTN','Timken','INA','Other'].map(t=>`<option ${(m.bearing_nde_make||'')=== t?'selected':''}>${t}</option>`).join('')}</select>
      </div>
      <div class="matrix-row"><label>NDE Type</label>
        <select id="mbnde_type_${m.id}">${['Ball','Roller','Spherical','Tapered','Needle','Other'].map(t=>`<option ${(m.bearing_nde_type||'')=== t?'selected':''}>${t}</option>`).join('')}</select>
      </div>
      <div class="motor-section-title">📊 Fault Frequencies</div>
      <div class="fault-freq-info">Used for bearing fault detection in spectrum analysis</div>
      ...
      <div class="matrix-row"><label>Sensor Type</label>
        <select id="ms_${m.id}">${['Vibration','Temperature','Vibration + Temperature','Ultrasound','Magnetic Flux','Pressure','RPM','All Sensors'].map(s=>`<option ${m.sensor_type===s?'selected':''}>${s}</option>`).join('')}</select>
      </div>
      <div class="matrix-row"><label>Alert Threshold</label><input id="ma_${m.id}" value="${m.alert_threshold||''}" placeholder="85°C / 10mm/s" /></div>
      <div class="motor-section-title">☁️ Data Collection Timing</div>
      <div class="timing-info">⚡ Node will follow these settings from cloud</div>
      <div class="matrix-row"><label>Overall Active</label>
        <select id="moverall_${m.id}" class="timing-select">${[{l:'5 Minutes',v:'5min'},{l:'15 Minutes',v:'15min'},{l:'30 Minutes',v:'30min'},{l:'1 Hour',v:'1hr'},{l:'2 Hours',v:'2hr'},{l:'4 Hours',v:'4hr'},{l:'6 Hours',v:'6hr'},{l:'8 Hours',v:'8hr'},{l:'12 Hours',v:'12hr'},{l:'16 Hours',v:'16hr'},{l:'18 Hours',v:'18hr'},{l:'24 Hours',v:'24hr'}].map(o=>`<option value="${o.v}" ${(m.overall_time||'5min')===o.v?'selected':''}>${o.l}</option>`).join('')}</select>
      </div>
      <div class="matrix-row"><label>Spectrum Active</label>
        <select id="mspectrum_${m.id}" class="timing-select">${[{l:'15 Minutes',v:'15min'},{l:'30 Minutes',v:'30min'},{l:'1 Hour',v:'1hr'},{l:'2 Hours',v:'2hr'},{l:'4 Hours',v:'4hr'},{l:'6 Hours',v:'6hr'},{l:'8 Hours',v:'8hr'},{l:'12 Hours',v:'12hr'},{l:'16 Hours',v:'16hr'},{l:'18 Hours',v:'18hr'},{l:'24 Hours',v:'24hr'}].map(o=>`<option value="${o.v}" ${(m.spectrum_time||'15min')===o.v?'selected':''}>${o.l}</option>`).join('')}</select>
      </div>
      <div class="matrix-row"><label>Sleep After</label>
        <select id="msleep_${m.id}" class="timing-select">${[{l:'No Sleep',v:'none'},{l:'30 sec',v:'30s'},{l:'1 Minute',v:'1min'},{l:'5 Minutes',v:'5min'},{l:'10 Minutes',v:'10min'},{l:'30 Minutes',v:'30min'},{l:'1 Hour',v:'1hr'}].map(o=>`<option value="${o.v}" ${(m.sleep_after||'none')===o.v?'selected':''}>${o.l}</option>`).join('')}</select>
      </div>
...
function previewCloudConfig(id) { updateCloudPreview(id); toast('☁️ Config preview updated!','info'); }
