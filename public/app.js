...
async function renderDashboard() {
  ...
  document.getElementById('statsGrid').innerHTML = cards.map(c =>      <div class="stat-card" style="--g:${c.g}">       <div class="stat-label">${c.icon} ${c.label}</div>       <div class="stat-value">${c.value}</div>       <div class="stat-sub">${c.sub}</div>     </div>).join('');
  initCharts();
}
function initCharts() {
...
function renderGateways() {
  document.getElementById('gwBody').innerHTML = gateways.length === 0
    ? 'No gateways yet.'
    : gateways.map(g => <tr>         <td><span class="badge badge-blue">${g.model}</span></td>         <td class="mono">${g.serial_no}</td>         <td class="mono">${g.imei||'-'}</td>         <td class="mono">${g.radio_mac||'-'}</td>         <td class="mono">${g.lan_mac||'-'}</td>         <td class="mono">${g.wan_mac||'-'}</td>         <td class="mono">${g.ble_mac||'-'}</td>         <td><span class="badge badge-gray">${g.frequency||'-'}</span></td>         <td>${g.site||'-'}</td>         <td><span class="badge ${g.status==='active'?'badge-green':'badge-red'}">${g.status||'active'}</span></td>         <td><button class="btn-icon" onclick="deleteGateway(${g.id})">🗑️</button></td>       </tr>).join('');
}
...
function renderNodes() {
  ...
  document.getElementById('nodeGateway').innerHTML = 'None' + gwOpts;
  document.getElementById('nodeBody').innerHTML = nodes.length === 0
    ? 'No nodes yet.'
    : nodes.map(n => <tr>         <td>           <div style="display:flex;align-items:center;gap:8px;">             <span style="display:inline-flex;">${NODE_ICON_SVG}</span>             <span class="badge badge-purple">${n.model}</span>           </div>         </td>         <td class="mono">${n.serial_no}</td>         <td class="mono">${n.radio_mac||'-'}</td>         <td class="mono">${n.ble_mac||'-'}</td>         <td><span class="badge badge-gray">${n.frequency||'-'}</span></td>         <td>${n.is_ai?'<span class="badge badge-ai">🤖 AI</span>':'<span class="badge badge-gray">Standard</span>'}</td>         <td>${n.gateway_model?${n.gateway_model}:'-'}</td>         <td>${n.site||'-'}</td>         <td><span class="badge ${n.status==='active'?'badge-green':'badge-red'}">${n.status||'active'}</span></td>         <td style="display:flex;gap:6px;align-items:center;">           <button class="btn btn-sm btn-outline" onclick="openCongregation(${n.id})">⚙️ Config</button>           <button class="btn btn-sm" style="background:#f59e0b;color:#fff;" onclick="openNodeData(${n.id})">📊 Data</button>           <button class="btn-icon" onclick="deleteNode(${n.id})">🗑️</button>         </td>       </tr>).join('');
}
...
function openNodeData(nodeId) {
  ...
  modal.innerHTML =      <div class="modal" style="max-width:960px;width:96%;max-height:92vh;">       <div class="modal-header" style="background:linear-gradient(135deg,#0f172a,#1e1b4b);color:#fff;border-radius:16px 16px 0 0;padding:16px 24px;">         <div style="display:flex;align-items:center;gap:12px;">           <div style="width:40px;height:40px;background:rgba(99,102,241,0.3);border-radius:10px;display:flex;align-items:center;justify-content:center;">${NODE_ICON_SVG}</div>           <div>             <h3 style="color:#fff;font-size:1rem;margin:0;">📊 ${currentNodeForData.model} — ${currentNodeForData.serial_no}</h3>             <p style="color:rgba(255,255,255,0.5);font-size:0.72rem;margin:2px 0 0;">NeuroVibe AI Technologies Pvt. Ltd.</p>           </div>         </div>         <button class="modal-close" style="color:#fff;background:rgba(255,255,255,0.1);" onclick="closeNodeData()">✕</button>       </div>       <div style="padding:16px;overflow-y:auto;max-height:80vh;">         <div class="nd-control-bar">           <div class="nd-ctrl-group">             <label>📋 View Mode</label>             <div style="display:flex;gap:5px;flex-wrap:wrap;">               <button class="data-tab active" onclick="switchDataTab('overall',this)">📈 Overall</button>               <button class="data-tab" onclick="switchDataTab('spectrum',this)">🌊 Spectrum</button>               <button class="data-tab" onclick="switchDataTab('rms',this)">📉 RMS</button>               <button class="data-tab" onclick="switchDataTab('water',this)">💧 Water</button>               <button class="data-tab" onclick="switchDataTab('ptp',this)">🎯 Pick to Pick</button>             </div>           </div>           <div class="nd-ctrl-group">             <label>🔌 Sensor</label>             <select id="sensorSelect" onchange="onSensorChange()" class="nd-select">               <option value="all">All Sensors</option>${senOpts}             </select>           </div>           <div class="nd-ctrl-group">             <label>⏱️ Interval</label>             <select id="dataIntervalSel" onchange="onIntervalChange()" class="nd-select">${intOpts}</select>           </div>           <div style="display:flex;align-items:center;gap:6px;">             <div class="live-dot"></div>             <span style="font-size:0.78rem;color:var(--success);font-weight:700;">LIVE</span>           </div>         </div>         <div class="nd-download-bar">           <span style="font-size:0.82rem;font-weight:700;">📥 Download:</span>           <select id="dlYear" class="nd-select">${yearOpts}</select>           <select id="dlMonth" class="nd-select">${monthOpts}</select>           <select id="dlSensor" class="nd-select"><option value="all">All Sensors</option>${senOpts}</select>           <select id="dlFormat" class="nd-select"><option value="csv">CSV</option><option value="json">JSON</option></select>           <button class="btn btn-primary btn-sm" onclick="downloadNodeData()">⬇ Download</button>         </div>         <div id="tab-overall" class="data-tab-content active">           <div class="nd-charts-grid" id="overallGrid"></div>           <div class="nd-live-grid" id="liveValuesGrid"></div>         </div>         <div id="tab-spectrum" class="data-tab-content">           <div class="nd-chart-card" style="margin-bottom:14px;">             <div class="nd-chart-title">🌊 Vibration Spectrum (FFT)</div>             <canvas id="spectrumChart" height="140"></canvas>           </div>           <div class="nd-chart-card">             <div class="nd-chart-title">🔊 Ultrasound Spectrum</div>             <canvas id="ultraChart" height="140"></canvas>           </div>         </div>         <div id="tab-rms" class="data-tab-content">           <div class="nd-rms-grid" id="rmsGrid"></div>         </div>         <div id="tab-water" class="data-tab-content">           <div class="nd-water-grid" id="waterGrid"></div>         </div>         <div id="tab-ptp" class="data-tab-content">           <div style="margin-bottom:12px;">             <h4 style="font-size:0.95rem;font-weight:700;">🎯 Pick to Pick Analysis</h4>             <p style="font-size:0.78rem;color:var(--muted);">Peak (Max) aur Trough (Min) ka difference</p>           </div>           <div class="nd-ptp-grid" id="ptpGrid"></div>           <div class="nd-chart-card" style="margin-top:14px;">             <div class="nd-chart-title">🎯 Pick to Pick Trend</div>             <canvas id="ptpChart" height="130"></canvas>           </div>         </div>       </div>     </div>;
...
function renderOverallCharts() {
  ...
  grid.innerHTML = list.map(s =>      <div class="nd-chart-card">       <div class="nd-chart-title" style="color:${s.color};">${s.label} (${s.unit})</div>       <canvas id="chart_${s.key}" height="100"></canvas>     </div>).join('');
  list.forEach(s => {
...
function renderRmsCards() {
  ...
  if (!grid) return;
  grid.innerHTML = SENSOR_LIST.map(s =>      <div class="nd-rms-card" style="border-top:3px solid ${s.color};">       <div class="nd-rms-label">${s.label} RMS</div>       <div class="nd-rms-val" id="rms_${s.key}" style="color:${s.color};">--</div>       <div class="nd-rms-unit">${s.unit}</div>       <canvas id="rmsChart_${s.key}" height="60"></canvas>     </div>).join('');
...
function renderWaterCards() {
  ...
  if (!grid) return;
  grid.innerHTML = SENSOR_LIST.map(s =>      <div class="nd-water-card">       <div class="nd-water-label" style="color:${s.color};">${s.label}</div>       <div class="nd-water-wrap">         <div class="nd-water-tank">           <div class="nd-water-fill" id="wf_${s.key}" style="height:50%;background:linear-gradient(180deg,${s.color},${s.color}88);"></div>           <div id="wb_${s.key}" style="position:absolute;inset:0;pointer-events:none;"></div>         </div>         <div class="nd-water-scale">           <span>${s.max}</span><span>${(s.max*0.75).toFixed(0)}</span><span>${(s.max*0.5).toFixed(0)}</span><span>${(s.max*0.25).toFixed(0)}</span><span>0</span>         </div>       </div>       <div class="nd-water-val" id="wv_${s.key}" style="color:${s.color};">-- ${s.unit}</div>     </div>).join('');
}
function renderPtpCards() {
  const grid = document.getElementById('ptpGrid');
  if (!grid) return;
  grid.innerHTML = SENSOR_LIST.map(s =>      <div class="nd-ptp-card" style="border-left:4px solid ${s.color};">       <div style="font-size:0.8rem;font-weight:700;color:${s.color};margin-bottom:8px;">${s.label}</div>       <div style="display:flex;gap:8px;margin-bottom:8px;">         <div class="nd-ptp-stat"><div class="nd-ptp-lbl">Peak</div><div class="nd-ptp-val" id="ptpMax_${s.key}" style="color:${s.color};">--</div></div>         <div class="nd-ptp-stat"><div class="nd-ptp-lbl">Trough</div><div class="nd-ptp-val" id="ptpMin_${s.key}" style="color:#64748b;">--</div></div>         <div class="nd-ptp-stat"><div class="nd-ptp-lbl">P-P</div><div class="nd-ptp-val" id="ptpPP_${s.key}" style="color:#6366f1;">--</div></div>         <div class="nd-ptp-stat"><div class="nd-ptp-lbl">Unit</div><div class="nd-ptp-val" style="color:#64748b;">${s.unit}</div></div>       </div>       <div style="height:5px;background:var(--border);border-radius:3px;overflow:hidden;">         <div id="ptpBar_${s.key}" style="height:100%;background:${s.color};width:50%;border-radius:3px;transition:width 0.5s;"></div>       </div>     </div>).join('');
  setTimeout(() => {
    ...
    nodeDataCharts.ptp = new Chart(el, {
      type:'line',
      data:{labels:Array.from({length:20},(_,i)=>${i*5}min), datasets:SENSOR_LIST.map(s=>({label:s.label, data:Array.from({length:20},()=>+(Math.random()s.max0.3).toFixed(2)), borderColor:s.color, backgroundColor:'transparent', tension:0.4, pointRadius:3, borderWidth:2}))},
...
function updateLiveData() {
  ...
  if (lv) {
    lv.innerHTML = SENSOR_LIST.map(s => {
      ...
      return <div class="nd-live-card">         <div style="display:flex;justify-content:space-between;align-items:center;font-size:0.8rem;margin-bottom:7px;">           <span style="font-weight:600;">${s.label}</span>           <strong style="color:${s.color}">${vals[s.key]} ${s.unit}</strong>         </div>         <div style="height:6px;background:var(--border);border-radius:3px;overflow:hidden;">           <div style="width:${pct}%;height:100%;background:${s.color};border-radius:3px;transition:width 0.5s;"></div>         </div>       </div>;
    }).join('');
  }
...
function downloadNodeData() {
  ...
  const data=allNodeData.length>0?allNodeData:Array.from({length:10},(_,i)=>({ts:Date.now()-i60000,...Object.fromEntries(SENSOR_LIST.map(s=>[s.key,+(Math.random()(s.max-s.min)+s.min).toFixed(2)]))}));
  if (format==='csv') {
...
async function openCongregation(nodeId) {
  ...
  document.getElementById('congNodeInfo').innerHTML =      <div class="node-icon" style="display:flex;align-items:center;justify-content:center;width:56px;height:56px;background:linear-gradient(135deg,#6366f1,#8b5cf6);border-radius:14px;">${NODE_ICON_SVG}</div>     <div>       <h3 style="font-size:1rem;font-weight:700;">${congNode.model} — ${congNode.serial_no}</h3>       <p style="color:var(--muted);font-size:0.84rem;margin-top:5px;">         ${congNode.frequency} &nbsp;|&nbsp;         ${congNode.is_ai?'<span class="badge badge-ai">🤖 AI Model</span>':'<span class="badge badge-gray">Standard</span>'}         &nbsp;|&nbsp; Radio: <span class="mono">${congNode.radio_mac||'-'}</span>         &nbsp;|&nbsp; BLE: <span class="mono">${congNode.ble_mac||'-'}</span>       </p>     </div>;
  renderMotors();
...
function renderMotors() {
  ...
  if (congMotors.length === 0) {
    grid.innerHTML = '⚙️No motors assigned. Click "+ Add Motor".';
    return;
  }
...
function addPinToMap(pin) {
  ...
  el.innerHTML=     <div class="pin-marker ${pin.type}" title="${pin.label}">       ${pin.type==='gw'?'📡':NODE_ICON_SVG}     </div>     <div class="pin-label">${pin.label}</div>     <div class="pin-actions">       ${pin.type==='nd'?⚙️:''}       ${pin.type==='nd'?✋:''}       ${pin.type==='nd'?📊:''}       ${pin.type==='gw'?👁️:''}       <span class="pin-btn del-btn" onclick="deletePin(${pin.id})">🗑️</span>     </div>;
  if (pin.type==='nd') {
...
function showGatewayNodes(gatewayId) {
  ...
  popup.innerHTML=     <div class="gw-popup-header">       <h4>📡 ${gw?gw.model:'Gateway'} — ${gw?gw.serial_no:''}</h4>       <button onclick="document.getElementById('gwPopup').remove()">✕</button>     </div>     <div class="gw-popup-body">       ${gwNodes.length===0?'<p style="color:var(--muted);text-align:center;padding:20px;">No nodes linked</p>'         :gwNodes.map(n=>
          ${n.model}
          ${n.serial_no}
          ${n.is_ai?'🤖 AI':''}
          ${n.status}
...
document.getElementById('mapCanvas').addEventListener('click', e=>{
  ...
  const x=((e.clientX-rect.left)/rect.width100).toFixed(1);
  const y=((e.clientY-rect.top)/rect.height100).toFixed(1);
  if(movingPinId){
...
async function loadUsers() {
  ...
  document.getElementById('userBody').innerHTML=users.map(u=>     <tr>       <td><strong>${u.name}</strong></td>       <td>${u.email}</td>       <td><span class="badge ${u.role==='admin'?'badge-purple':'badge-gray'}">${u.role}</span></td>       <td style="font-size:0.78rem;color:var(--muted);">${new Date(u.created_at).toLocaleDateString('en-IN')}</td>       <td>${u.email!=='admin@neurovibe.ai'?🗑️:'-'}</td>     </tr>).join('');
}
